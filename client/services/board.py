...
class Board(BaseService):
    ...
    @player_turn.setter
    def player_turn(self, turn: bool) -> None:
    ...
    def setBoardByFEN(self, fen: str):
        ...
        self.player_turn = True if items[1].lower() == 'w' else False
        self.ai_turn = not self.player_turn
    ...
    def handle_moving(self, pos: tuple[int, int]):
        ...
        if len(clicked_sprites) > 0:
            if clicked_sprites[0].is_white != self.player_turn and self.selectedPiece is None:
                return
    ...
    def next_turn(self):
        ...
        if self.is_ai and self.player_turn == self.ai_turn:
    ...
    def _is_piece_move_from_human(self, piece):
    ...
    def make_turn(self, dest: tuple[int, int], test_mode=False, extra="") -> None | Piece:
        ...
        if not test_mode:
            ...
            direct = self._is_piece_move_from_human(from_piece)
            square_name = self.create_square_name(from_piece, dest, direct, extra)
        ...
        if extra != "" and isinstance(from_piece, Pawn):
            ...
            i_x, i_y, is_white = from_piece.x, from_piece.y, from_piece.is_white
            if self._is_piece_move_from_human(from_piece):
                i_x, i_y, is_white = from_piece.prev_x, from_piece.prev_y, from_piece.is_white
            ...
            from_piece = self.coordinate[i_x, i_y]
    ...
    def make_turn_by_square_notation(self, notation):
        ...
        self.make_turn((to_x, to_y), False, extra.upper())
        self.next_turn()
    ...
    def create_square_name(self, piece: Piece, dest: tuple[int, int], direct: bool, extra=""):
        x, y = dest[0], dest[1]

        long = Board.row_notation[piece.x] + Board.col_notation[piece.y]

        pos = Board.row_notation[x] + Board.col_notation[y]
        if extra != "" and not direct:
    ...
    def filter_invalid_moves(self) -> None:
        ...
        team_king = white_king if self.player_turn else black_king
        opponents = [p for p in self.pieces if p.is_white != self.player_turn]
        teams = [p for p in self.pieces if p.is_white == self.player_turn]
    ...
    def _filter_illegal_castling(king_piece: King, opponent_piece: Piece):
        ...
        if king_piece.x == 6 and king_piece.can_castle_king:
            illegal_moves += [move for move in opponent_piece.possibleMoves if move.x == 5
                              and move.y == king_piece.y]
        if king_piece.x == 4 and king_piece.can_castle_queen:
            illegal_moves += [move for move in opponent_piece.possibleMoves if move.x == 3
                              and move.y == king_piece.y]
